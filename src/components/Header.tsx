...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header ref={searchContainerRef} className="sticky top-0 z-40 w-full border-b border-border glass transition-colors duration-300">
      <div className="mx-auto flex h-14 sm:h-16 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8 py-1.5 gap-2 sm:gap-4">
        ...
        {/* Desktop Responsive Search Bar & Instant Autocomplete Dropdown (Hidden on Mobile < md) */}
        <form 
        ...
        >
          ...
          {/* Autocomplete Predictive Live Results Modal (Desktop) */}
      ...
      {/* Mobile Dedicated Full-Width Search Input & Results (Visible on Mobile < md) */}
      <div className="block md:hidden px-3 pb-2.5 pt-0.5 border-t border-border/30 bg-background/50">
        <form onSubmit={handleSearchSubmit} className="relative w-full">
          <div className="relative w-full">
            <input
              type="text"
              placeholder="Search tools, hardware & materials..."
              value={searchVal}
              onFocus={() => setIsSearchFocused(true)}
              onChange={handleSearchInputChange}
              className="w-full rounded-full border border-border bg-background/80 pl-9 pr-8 py-1.5 text-xs text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-all shadow-xs"
            />
            ...
            {searchVal && (
              <button
                ...
                className="absolute right-3 top-1.5 text-xs text-muted-foreground hover:text-foreground font-bold p-0.5"
              >
                ✕
              </button>
            )}
          ...
          {/* Autocomplete Predictive Live Results Modal (Mobile Full Width) */}
          {isSearchFocused && searchVal.trim().length >= 2 && (
            <div className="absolute top-full left-0 right-0 mt-1.5 rounded-2xl border border-border bg-card p-3 shadow-2xl z-50 animate-in fade-in slide-in-from-top-2 duration-150 max-h-[65vh] overflow-y-auto">
              <div className="flex items-center justify-between px-2 pb-2 border-b border-border/50 mb-2">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-muted-foreground">
                  Matching Products ({matchingSearchResults.length})
                </span>
                  ...
                  {matchingSearchResults.map((prod) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs font-black text-primary">₹{prod.price.toLocaleString('en-IN')}</span>
                          <span className="text-[9px] text-muted-foreground uppercase truncate">{prod.category}</span>
                        </div>
                      </div>
                      <ArrowRight className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                    </div>
