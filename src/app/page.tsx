...
export default function Home() {
  ...
  // Load catalog and optionally seed Firestore on first run
  useEffect(() => {
    ...
    // Only seed Firestore when real Firebase credentials are present.
    // seedDatabase() has an internal guard that skips if data already exists.
    if (!isMock) {
      const runSeed = async () => {
        setIsSeeding(true);
        await seedDatabase();
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (p) => 
          ...
          (p.searchKeywords && p.searchKeywords.some(kw => kw.toLowerCase().includes(q))) ||
          (p.seo?.metaKeywords && p.seo.metaKeywords.some(kw => kw.toLowerCase().includes(q)))
      );
    }
    ...
    if (selectedPriceRange) {
      switch (selectedPriceRange) {
        case 'under_499':
          ...
          break;
        case '499_1999':
          result = result.filter((p) => p.price >= 499 && p.price <= 1999);
          break;
        case '2000_4999':
          result = result.filter((p) => p.price >= 2000 && p.price <= 4999);
          break;
        case 'over_5000':
