      ...
      try {
        ...
        if (cashfreeInstance) {
          try {
            ...
            console.warn('Cashfree checkout error on localhost:', checkoutErr);
            alert('Localhost Domain Notice: http://localhost:3000/ is not a whitelisted Cashfree domain. Completing test order for local verification...');
            completeOrderPlacement();
          }
        } else {
        ...
        if (window.location.hostname.includes('localhost') || window.location.hostname.includes('127.0.0.1')) {
          alert('Localhost Testing Fallback: ' + (err.message || 'http://localhost:3000/ domain whitelisting is required by Cashfree. Completing order in Local Test Mode...'));
          completeOrderPlacement();
        } else {
