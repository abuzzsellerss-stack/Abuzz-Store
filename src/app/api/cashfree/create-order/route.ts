...
export async function POST(req: NextRequest) {
  try {
    ...
    // If running locally on localhost with PRODUCTION credentials, trigger dev mode response to avoid Cashfree unwhitelisted domain error page
    if (isLocalhost && env === 'PRODUCTION') {
      console.log(`[CASHFREE LOCALDEV NOTICE]: Localhost detected with Production credentials. Providing local dev session for ${uniqueOrderId}`);
      return NextResponse.json({
        success: true,
